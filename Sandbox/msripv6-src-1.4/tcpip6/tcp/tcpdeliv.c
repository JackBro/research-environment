    ...
    TCPRcvReq *FreedReq)  // Rcv request structure to be freed.
{
    ...
    BufferLink = CONTAINING_RECORD(&(FreedReq->trr_next), SINGLE_LIST_ENTRY,
                                   Next);
    ...
    uint Size)             // Maximum size in bytes to seize.
{
    ...
    if (!(OrigPkt->Flags & PACKET_OURS)) {
        ...
        FirstPkt = ExAllocatePool(NonPagedPool,
                                  sizeof(IPv6Packet) + BytesToCopy);
        if (FirstPkt != NULL) {
            EndPkt = FirstPkt;
            FirstPkt->Next = NULL;
            FirstPkt->Position = 0;
            FirstPkt->Data = (uchar *)(FirstPkt + 1);
            ...
            while (OrigPkt != NULL && !(OrigPkt->Flags & PACKET_OURS)
                ...
                BytesToCopy = MIN(Size, OrigPkt->TotalSize);
                NewPkt = ExAllocatePool(NonPagedPool,
                                        sizeof(IPv6Packet) + BytesToCopy);
                if (NewPkt != NULL) {
                    NewPkt->Next = NULL;
                    NewPkt->Position = 0;
                    NewPkt->Data = (uchar *)(NewPkt + 1);
    ...
    uint Size)             // Size in bytes of data in InPacket.
{
    ...
    do {
        ...
        if ((SrcSize -= BytesToCopy) == 0) {
            ...
            if (SrcPkt->ContigSize == SrcPkt->TotalSize) {
                //
                // We have a single contiguous region, easy to skip forward.
                //
                SrcPkt->ContigSize -= BytesToCopy;
                SrcPkt->TotalSize -= BytesToCopy;
                (uchar *)SrcPkt->Data += BytesToCopy;
                SrcPkt->Position += BytesToCopy;
            } else {
                //
                // REVIEW: This method isn't very efficient.
                //
                GetDataAtPacketOffset(SrcPkt, BytesToCopy, &SrcPkt->Data,
                                      &SrcPkt->ContigSize);
                SrcPkt->TotalSize -= BytesToCopy;
                SrcPkt->Position += BytesToCopy;
            }
    ...
    if (DestReq != NULL) {
        ...
        DestReq->trr_offset = DestPtr - 
            (uchar *) NdisBufferVirtualAddress(DestBuf);
    ...
    uint Size)             // Size in bytes of data in InPacket.
{
    ...
    if (RcvReq != NULL) {
        ...
        IF_TCPDBG(TCP_DEBUG_RECEIVE) {
            KdPrint(("Indicating %lu bytes, %lu available\n",
                     InPacket->ContigSize, Size));
        }
        ...
        //
        // BUGBUG: Can't indicate more than the contiguous data via this API.
        //
        Status = (*Event)(EventContext, ConnContext, IndFlags,
                          InPacket->ContigSize, Size, &BytesTaken,
                          InPacket->Data, &ERB);

        IF_TCPDBG(TCP_DEBUG_RECEIVE) {
            KdPrint(("%lu bytes taken, status %lx\n", BytesTaken, Status));
        }
    ...
    KIRQL PreLockIrql)  // IRQL prior to acquiring TCB lock.
{
    ...
    for (;;) {
        ...
        IF_TCPDBG(TCPDebug & TCP_DEBUG_RECEIVE) {
            KdPrint(("Indicating pending %d bytes, %d available\n",
                     RcvTCB->tcb_pendhead->ContigSize,
                     RcvTCB->tcb_pendingcnt));
        }
        ...
        Status = (*Event)(EventContext, ConnContext,
                          ...
                          RcvTCB->tcb_pendhead->Data, &ERB);

        IF_TCPDBG(TCPDebug & TCP_DEBUG_RECEIVE) {
            KdPrint(("%d bytes taken\n", BytesTaken));
        }
        ...
        if (Status == TDI_MORE_PROCESSING) {
            ...
            IF_TCPDBG(TCP_DEBUG_RECEIVE) {
                KdPrint(("more processing on receive\n"));
            }
    ...
    do {
        ...
        if (RcvReq != NULL) {
            ...
            if (RcvTCB->tcb_urgind == 0) {
                ...
                KeAcquireSpinLock(&ConnTableLock, &Irql2);
                KeAcquireSpinLock(&RcvTCB->tcb_lock, pTCBIrql);
                if ((Conn = RcvTCB->tcb_conn) != NULL) {
                    ...
                    if (Conn->tc_ao != NULL) {
                        ...
                        if (AO_VALID(AO) && (ExpRcv = AO->ao_exprcv) != NULL) {
                            ...
                            Status = (*ExpRcv)(ExpRcvContext, ConnContext,
                                               ...
                                               RcvPkt->ContigSize, Size,
                                               &BytesTaken, RcvPkt->Data,
                                               &ERB);
                    ...
                    KeReleaseSpinLock(&ConnTableLock, *pTCBIrql);
    ...
    IPv6Packet **ThirdPkt)   // Where to return third packet pointer.
{
    ...
    if (SecondPkt != NULL) {
        ...
        //
        Temp = ExAllocatePool(NonPagedPool, sizeof(IPv6Packet) + SecondSize);
        if (Temp != NULL) {
            Temp->Next = NULL;
            Temp->Position = 0;
            Temp->Data = (uchar *)(Temp + 1);
    ...
    if (ThirdPkt != NULL) {
        ...
        Temp = ExAllocatePool(NonPagedPool, sizeof(IPv6Packet) + ThirdSize);
