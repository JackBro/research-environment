...
NDIS_HANDLE DGPacketPool, DGBufferPool;
    ...
    DGRcvReq *RcvReq)  // RcvReq to be freed.
{
    ...
    BufferLink = CONTAINING_RECORD(&(RcvReq->drr_q.q_next), SINGLE_LIST_ENTRY,
                                   Next);
    ...
    PNDIS_PACKET Packet)  // Packet that was sent.
{
    ...
    //
    NDIS_BUFFER_LINKAGE(BufferChain) = NULL;  // REVIEW: Needed?
    ...
    for (qentry = QNEXT(&(SrcAO->ao_sendq)); qentry != &(SrcAO->ao_sendq);
        ...
        if (sendReq->dsr_context == Context) {
            ...
            IF_TCPDBG(TCP_DEBUG_SEND_DGRAM) {
                KdPrint(("TdiCancelSendDatagram: Dequeued item %lx\n",
                         Context));
            }
    ...
    PVOID Context)
{
    ...
    for (qentry = QNEXT(&(SrcAO->ao_rcvq)); qentry != &(SrcAO->ao_rcvq);
        ...
        if (rcvReq->drr_context == Context) {
            ...
            IF_TCPDBG(TCP_DEBUG_SEND_DGRAM) {
                KdPrint(("TdiCancelReceiveDatagram: Dequeued item %lx\n",
                         Context));
            }
    ...
    PNDIS_BUFFER Buffer)                     // Buffer chain to fill in.
{
    ...
    if (AO_VALID(RcvAO)) {
        ...
        IF_TCPDBG(TCP_DEBUG_RAW) {
            KdPrint(("posting receive on AO %lx\n", RcvAO));
        }
...
InitDG(void)
{
    ...
    //
    // Initialize Datagram Packet and Buffer pools.
    //
    NdisAllocatePacketPool(&Status, &DGPacketPool, DG_MAX_HDRS,
                           sizeof(Packet6Context));
    if (Status != NDIS_STATUS_SUCCESS)
        return FALSE;
    NdisAllocateBufferPool(&Status, &DGBufferPool, DG_MAX_HDRS);
    if (Status != NDIS_STATUS_SUCCESS) {
        NdisFreePacketPool(DGPacketPool);
        return FALSE;
    }
