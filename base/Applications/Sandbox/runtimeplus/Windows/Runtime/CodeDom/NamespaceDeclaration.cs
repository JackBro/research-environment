...
namespace System.Extensions.CodeDom
{
	...
	public class NamespaceDeclaration : Declaration
	{
		...
		public NamespaceDeclaration(string name):base(name, new NameConformer())
		{
			this.imports.Add("System");
		}

		public NamespaceDeclaration(string name, NameConformer conformer):base(name,conformer)
		{
			this.imports.Add("System");
		}
		...
		public ClassDeclaration AddClass(string name, MemberAttributes attributes)
		{
			if (name==null)
				throw new ArgumentNullException("name");
			if (this.Classes.Contains(name))
		...
		public IDictionary ToCodeDom()
		{
			...
			foreach(ClassDeclaration c in this.classes.Values)
			{
				...
				foreach(String s in this.imports)
					usings.Add(s);
				foreach(String import in c.GetImports())
				{
