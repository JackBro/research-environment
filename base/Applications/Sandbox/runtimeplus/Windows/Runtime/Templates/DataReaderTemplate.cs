...
namespace System.Extensions.Templates
{
	...
	/// </summary>
	public class DataReaderTemplate : ITemplate
	{
		...
		public void Generate(System.Extensions.CodeDom.NamespaceDeclaration ns)
		{
			...
			// generate data
			this.Data.Generate(ns);

			// generate the rest
			ns.AddImport("System.Data");

			// create class
			ClassDeclaration c = ns.AddClass(this.Name + "DataReader");
			...
			FieldDeclaration data = c.AddField(
				DataTemplate.DecorateName(this.Data.Name)
				,"data");
			data.InitExpression = 
				Expr.New(data.Type);
			...
			ParameterDeclaration drp = cs.Signature.AddParam(dr.Type,"dr",false);
		...
		private void AddEnumerator(ClassDeclaration c, FieldDeclaration data, MethodDeclaration close)
		{
			...
			ParameterDeclaration collection = cs.Signature.AddParam(c,"collection",true);
