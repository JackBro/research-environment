...
namespace System.Extensions.CodeDom.Doc
{
	...
	public class XmlMarkup
	{
		...
		public XmlMarkup Add(string name, string text)
		{
			XmlElement el = this.doc.CreateElement(name);
			this.current.AppendChild(el);

			XmlText t = this.doc.CreateTextNode(text);
			el.AppendChild(t);
		...
		public XmlMarkup AddCDATA(string name, string text)
		{
			XmlElement el = this.doc.CreateElement(name);
			this.current.AppendChild(el);

			XmlCDataSection t = this.doc.CreateCDataSection(text);
			el.AppendChild(t);
