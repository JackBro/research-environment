include "../../../Trusted/Spec/Apps/DiffPriv/SumReducer_spec.dfy"
include "Mapper_impl.dfy"

method ComputeSum (db:seq<Row>, program:seq<Operation>, row_min:int, row_max:int,
    ...
    ensures answer == Clip(Scale(sum, answer_units), answer_min, answer_max);
{
    ...
    lemma_fundamental_div_mod_converse_pos(sum, answer_units, scaled_sum, total_remainder);
...
function method SaturatingAdd(x:int, y:int):int
    requires Word32(x);
    requires Word32(y);
...
function method ClipWord32 (value:int, min:int, max:int) : int
