...
static function method {:timeLimit 30} padded_len(msg_len:int) : int
	...
	ensures  padded_len(msg_len) == PaddedLength(msg_len);
{
	...
	min_padded_len(msg_len) + num_padding_zeroes(msg_len) + 0
}
...
// The following verifies successfully.  Body commented out to speed proofs
method pad_message_with_zeroes(data: array<int>, len: int)
	requires word(len);
	requires len <= Power2(32) - 1 && len < Power2(64);
	requires data != null;	
	requires data.Length * 32 >= len;
...
/* 
	...
	data[one_bit_index] := and(data[one_bit_index], not(sub2(left_shift(1, 31 - (len % 32)), 1)));	// Set remaining LSB bits of this int to 0
	lemma_bit_masks();
	
	ghost var amt := 31 - (len % 32);
	ghost var result := data[one_bit_index]; 
	assert(result == and(d, not(sub2(left_shift(1, amt), 1))));
	...
	ghost var num_zero_ints := padded_len(len) / 32 - (one_bit_index + 1);
	...
	ghost var num_padding_zeroes := num_zero_ints * 32 + num_zeroes - 64;
	assert num_padding_zeroes == NumPaddingZeroes(len);
...
ghost method lemma_seq_to_int_split(arr:seq<int>, singleton:seq<int>, len:int, amt_of_singleton:int)
	requires len % 32 == 0;
	requires len == |arr| * 32;
	requires len < Power2(32);
	...
	ensures  seq_to_int(arr + singleton, len + amt_of_singleton) == Power2(amt_of_singleton) * seq_to_int(arr, len) + seq_to_int(singleton, amt_of_singleton);
{
	...
	if (amt_of_singleton == 0) {
		...
		assert 1*seq_to_int(data, len_plus_amt) == Power2(amt_of_singleton) * seq_to_int(arr, len) + seq_to_int(singleton, amt_of_singleton);
		assert 1*seq_to_int(arr + singleton, len + amt_of_singleton) == 1* Power2(amt_of_singleton) * seq_to_int(arr, len) + seq_to_int(singleton, amt_of_singleton);
	}
}
...
ghost method lemma_a2i_is_int_decomposition(arr:seq<int>, len:int, num_bits_sub:int)
