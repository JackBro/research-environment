    ...
    */
	    ...
	    if(error !== undefined && error !== null) {
		    ...
		    for (name in error) {
		        if(name !== undefined) {
                    ...
                    var errorLine = error[name];
                    if(errorLine === undefined || errorLine === null ) {
                        errorLine = '';
                    }
		            errorLine = errorLine.toString().replace(/^\s*(\S*(\s+\S+)*)\s*$/, "$1");
		            message += (name + ': '  + errorLine);
		            message += '\n';
    ...
    function getErrorDetailsAsJson() {
        ...
        for (var i = 0; i<JSLINT.errors.length; i++) {
            ...
            var currentError = { Line: (e.line || '').toString().replace(/^\s*(\S*(\s+\S+)*)\s*$/, "$1"),
                Character: (e.character || '').toString().replace(/^\s*(\S*(\s+\S+)*)\s*$/, "$1"),
                Reason: (e.reason || '').toString().replace(/^\s*(\S*(\s+\S+)*)\s*$/, "$1"),
                Evidence : (e.evidence || '').toString().replace(/^\s*(\S*(\s+\S+)*)\s*$/, "$1") }; 
    ...
    function printAllErrorDetails() {
        WScript.StdErr.WriteLine("DETAILED ERROR MESSAGES");
        for(var i = 0; i<JSLINT.errors.length; i++) {
            var errorMessage = getErrorMessageFor(JSLINT.errors[i]);
    ...
    (function () {
        try
        {
            ...
            var resultStartDelim = '__START__ERROR__REPORT__';
            var resultEndDelim = '__END__ERROR__REPORT__';
            ...
            //first line feed in must the options as JSON
            var optionsStr = WScript.StdIn.ReadLine();
            WScript.StdErr.WriteLine('Options Str: ' + (optionsStr));
            var options = YAHOO.lang.JSON.parse(optionsStr);
            ...
            var jscriptToVerify = strRead;
            
            if(SKIPIT) {
                jscriptToVerify = 'function test()    {}';
            }
            
	        //if (!JSLINT(jscriptToVerify, {passfail: false})) {	
	        if (!JSLINT(jscriptToVerify, options)) {	
		        WScript.StdErr.WriteLine(resultStartDelim);
		        ...
		        WScript.StdErr.WriteLine(resultEndDelim);
        		
        		
    		    printAllErrorDetails();
        		
		        WScript.Quit(1);
	        }
            else {
                WScript.StdErr.WriteLine("__NO__JSLINT__ERRORS__");
            }
            ...
            WScript.StdErr.WriteLine('ERRORS:');
            WScript.StdErr.WriteLine((getErrorMessageFor(error)));
            WScript.Quit(-1);
        }
