...
for method=1:5
  ...
  labelpars={'fontname','helvetica','fontsize',16,'horiz','center'};
  flashpars={'fontname','helvetica','fontsize',25,'horiz','center'};
  titlepars={'fontname','helvetica','fontsize',32,'horiz','center'};
  lpars={'marker','.','linestyle','none','markersize',64};
  tpars={'fontname','helvetica','fontsize',16,'horiz','center','col','g'};
  for i=1:n
    subplot(1,2,2)
    spy(Bplt,16), xlabel('');
    nzhandle=text(50,114.9,['nz = ',int2str(nnz(Bplt))],flashpars{:});
    ...
    flophandle=text(5.5,-.25,['flops = ',int2str(nflops)],flashpars{:});
    titlehandle=text(11.5,11.2,methodstr,titlepars{:});
    ...
    hold on, spy(Btmp,'r',25), xlabel(''); hold off
    delete(nzhandle);
    if i==n
      nzhandle=text(50,114.9,['nz = ',int2str(nnz(Bplt))],flashpars{:}); 
    else
      nzhandle=text(50,113.25,['nz = ',int2str(nnz(Bplt))],labelpars{:});
    end
    ...
    if i==n
      ...
      flophandle=text(5.5,-.1,['flops = ',int2str(nflops)],labelpars{:});
    end
