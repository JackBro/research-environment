                    ...
                    'abuseDlgAlreadyReported':function (decisionMade){
                        if(decisionMade){
                            $('#abuse_msg').html('Moderators have determined that this comment does not violate PubMed Commons\' Comment Guidelines. For more information on PubMed Commons Guidelines,' + 
                                ' or to provide further feedback, please see <a href="/pubmedcommons/">https://www.ncbi.nlm.nih.gov/pubmedcommons/</a>. Thank you!');
                        }
                        else
...
Portal.Portlet.Pubmed_DisplayBar = Portal.Portlet.Entrez_DisplayBar.extend({
	...
	'SendToMail': function(sendto){
	    ...
	    // unhide email form
	    if (jQuery("#submenu_Mail").find("ul")){
	    	jQuery("#submenu_Mail").find("ul").removeClass('hidden');
	    }
        ...
        var descNode = document.getElementById("email_desc");
	    var subjNode = document.getElementById("email_subj");
        if (selectedItemCount > 0){
        ...
        if (document.getElementById("email_count_option")){
            if (count <= 5 || selectedItemCount > 1 || (!document.getElementById("dest_AddToClipboard") && count <= 200)){
                document.getElementById("email_count_option" ).style.display = "none";
            }
            else {
                document.getElementById("email_count_option").style.display = "list-item";
            }
        }
        if (document.getElementById("email_start_option")){
            if (count <= 5 || selectedItemCount > 1 || (!document.getElementById("dest_AddToClipboard") && count <= 200)){
                document.getElementById("email_start_option" ).style.display = "none";
            }
            else {
                document.getElementById("email_start_option").style.display = "list-item";
            }
        }
        
         // don't show sort option if 1 item is selected
        if (document.getElementById("email_sort_option")){
            if (count == 1){
                document.getElementById("email_sort_option").style.display = "none";
            }
            else {
                document.getElementById("email_sort_option").style.display = "list-item";
            }
        }
        
        // if you are in the clipboard and total is more than 200, then show warning
        if (selectedItemCount < 1 && !document.getElementById("dest_AddToClipboard") && count > 200){
            var alertnode = document.getElementById("email_alert");
	        alertnode.innerHTML = 'Only 200 items will be sent to avoid exceeding email limitations.';
	        alertnode.className = 'email_alert';
	        document.getElementById("email_count").value = "200";
        } 
        // if you are in the clipboard and total is less than 200, make count 200 to send everything
        else if (selectedItemCount < 1 && !document.getElementById("dest_AddToClipboard")){
            if (document.getElementById("email_count")){
                document.getElementById("email_count").value = "200";
            }
        }
        
        if (document.getElementById("email_check1")){
        	document.getElementById("email_check1").value = "checked";
	    }
	    
	    // captcha is not shown if user is signed in to myncbi
		if (document.getElementById("captcha-segment")){
			console.info("CaptchaReady" + CaptchaReady);
			if (!CaptchaReady){
				//load captcha script
				var CaptchaScript = "https://www.google.com/recaptcha/api.js?onload=onCaptchaLoadCallback&render=explicit";
		    	jQuery.cachedScript( CaptchaScript) // add timeout here?
		    		.done(function( script, textStatus ){
					})
					.fail(function( jqxhr, settings, exception ) {
						jQuery("#email_alert").html("Please log in to your NCBI account to send email").attr('class', 'email_alert');
					});
			}
			else{
				this.GetCaptcha();
	...
	'SendToCitManager': function(name){
	    var count = this.getItemCount();
	    var selectedItemCount = getEntrezSelectedItemCount() || 0;
	    
	    // don't show number to send and start options if less than 5 items are in search result, or user has selected some items
        if (document.getElementById("citman_count_option")){
            if (count <= 5 || selectedItemCount > 1){
                document.getElementById("citman_count_option").style.display = "none";
            }
            else {
                document.getElementById("citman_count_option").style.display = "list-item";
            }
        }
        if (document.getElementById("citman_start_option")){
            if (count <= 5 || selectedItemCount > 1){
                document.getElementById("citman_start_option").style.display = "none";
            }
            else {
                document.getElementById("citman_start_option").style.display = "list-item";
            }
        }
	...
	'SendToSubmitted': function(cmd, idx, e, target, name){
	    if (cmd == 'mail'){
	         this.SendToEmailSubmitted(cmd, target);
	    }
	    else if (cmd == 'citationmanager'){
	         this.SendToCitManagerSubmitted(cmd, target);
	    }
	    else{
	...
	'SendToEmailSubmitted': function(cmd, target){
	    var alertnode = document.getElementById("email_alert");
	    alertnode.className = 'hidden';
	    ...
	    var email = document.getElementById("email_address" ).value.replace(/^\s*|\s*$/g,'');
	    if (email == ''){
    		...
    		if (emailRegexp.test(email)){   		    
	        	...
	        	if (document.getElementById("captcha-segment")){
	        		var captchaanswer = "";
	        		var captchaId = CaptchaId1;
	        		if (captchaId == undefined){
	        			jQuery("#email_alert").html("We could not verify that you are not a robot. Please sign in to your NCBI account to send email.").attr('class', 'email_alert');
	        		}
	        		else {
	        			try {
	        				captchaanswer = grecaptcha.getResponse(captchaId);
	        			}
	        			catch(e){
	        				jQuery("#email_alert").html("We could not verify that you are not a robot. Please sign in to your NCBI account to send email.").attr('class', 'email_alert');
	        			}
	        			if (captchaanswer != ''){
	        				this.SendMailInfo(cmd, target, email, captchaanswer);
	        			}
	        			else{
        	    	    	alertnode.innerHTML = 'Please verify that you are not a robot or log in to your NCBI account to send email';
	                        alertnode.className = 'email_alert';
	        			}
        			}
	        	} 
	        	else {
	        		this.SendMailInfo(cmd, target, email);
	        	}//captcha check            
    		}
    		else {
	...
	'SendMailInfo': function(cmd, target, email, captchaanswer){
	    // collect options, description and extra text
	    var emailFormat = document.getElementById("email_format");
	    var report = emailFormat.value;
	    var format =  emailFormat.options[emailFormat.selectedIndex].getAttribute('format');
	    var sort = document.getElementById("email_sort")? document.getElementById("email_sort").value : "";
	    var count = document.getElementById("email_count")? document.getElementById("email_count").value : "5";
	    var start = document.getElementById("email_start")? document.getElementById("email_start" ).value : "1";
	    var text = document.getElementById("email_add_text").value;
	    var subj = document.getElementById("email_subj").value;
	    var querykey = target.getAttribute('qk');
	    var querydesc = document.getElementById("email_desc").innerHTML;
	    ...
	    var emailcheck1 = document.getElementById("email_check1")? document.getElementById("email_check1").value : "";
	    var emailcheck2 = document.getElementById("email_check2")? document.getElementById("email_check2").value : "";
	    
	    
	    try{
	        jQuery("#email_alert").html("Processing request.").attr('class', 'email_alert');
			...
			jQuery("#email_alert").html("We are experiencing some problems with our server. Please try again later.").attr('class', 'email_alert');
			document.getElementById("email_submit").disabled = false;
		}
    ...
    'GetCaptcha':function(){
    	jQuery("#email_captcha_alert").html("loading...").attr('class', 'email_alert');				
    	var CaptchaId = CaptchaId1;
    	console.info("init captchaid " + CaptchaId);
    	try{
    		if (jQuery("#recaptcha_div").find("div") && CaptchaId != undefined){
    			console.info("reload captcha");
    			grecaptcha.reset(CaptchaId);
			}
    		else {
    			if(jQuery("#recaptcha_div").find("div")){
    				jQuery("#recaptcha_div").html("");
    			} 
		        var NCaptchaId;
		        NCaptchaId = grecaptcha.render(
			    				"recaptcha_div",
			    				{
			    					"sitekey": "6LeaXvkSAAAAABvJ8rWi-BttEjUDaLDE4nQbIu-3",
			    					"theme": "light"
			    				}
			  				); 
			  	console.info("captchaid after render: " + NCaptchaId + ".");
			  	console.info(NCaptchaId);
			  	if (NCaptchaId != undefined){
			  	}
			  	else{
			  		console.info("captchaid after render prob msg");
			  		jQuery("#email_alert").html("We are experiencing some problems verifying that you are not a robot. Please try again later or sign in to your NCBI account to send email.").attr('class', 'email_alert');		
			  		
			  	}   
			  	CaptchaId1 = NCaptchaId;
			}
		...
		jQuery("#email_captcha_alert").html("").attr('class', 'hidden');
    },
    ...
    'MailSubmissionResponse':function(data){
    	...
    	if (data.success == 'true'){
    	    jQuery("#email_alert").html("").attr('class', 'hide');
    		var msg = data.msg || "Message sent.";
	    	jQuery("#sendto").ncbipopper( 'close' );
	    	this.send.AddUserMessage({'type' : 'success', 'name' : 'email_m', 'msg' : msg});
    	}
    	else if (data.errortype == 'ajax' || data.errortype == 'email'){
    		this.send.RemoveUserMessage({'name' : 'email_m'});
    		jQuery("#email_alert").html(data.error).attr('class', 'email_alert');
	    	document.getElementById("email_submit").disabled = false;
    		// reissue new captcha
    		this.GetCaptcha();
    	}
    	    ...
    	    this.send.RemoveUserMessage({'name' : 'email_m'});
    		jQuery("#email_alert").html("We could not verify that you are not a robot. Please try again or sign in to NCBI.").attr('class', 'email_alert'); 
	    	document.getElementById("email_submit").disabled = false;
	    	//reissue new captcha
    		this.GetCaptcha();
    	}
    ...
    'SendToCitManagerSubmitted': function(cmd, target){
         this.setValue("FileFormat", 'nbib');
         this.setValue("FileSort", this.getValue("LastSort"));
         if (document.getElementById("citman_count_option") && document.getElementById("citman_count_option").style.display == "list-item"){
             document.getElementById("citman_customrange").value = "true";
             this.setValue("PageSize", document.getElementById("citman_count").value);
	         document.getElementById("citman_startindex").value = document.getElementById("citman_start").value;
	     }
...
var AbstractLink = function (){
	...
	function procSsid(currDb, term, sec){
		...
		if (currDb == "Nucleotide" && term.match(/^GENBANK\/(.+)/i) ){
    		...
    		return "https://www.ncbi.nlm.nih.gov/projects/geo/query/acc.cgi?acc=" + RegExp.$1;
  		}
...
/*  		else if (currDb.match(/PubChem\s+(\S+)/) && term.match(new RegExp('^PubChem-(' + RegExp.$1 + ')\/(\\d+)'))) {
			...
			rcTerm = RegExp.$2 + "[uid]";
		     ...
		     return "https://pubchem.ncbi.nlm.nih.gov/summary/summary.cgi?sid=" + RegExp.$1;
		}
		else if (currDb == "pcassay" && term.match(/^PubChem-BioAssay\/(\d+)/)){ //|| currDb == "pcassay" || currDb == "pccompound")
		     return "https://pubchem.ncbi.nlm.nih.gov/assay/assay.cgi?aid=" + RegExp.$1;
		}
  		    ...
  		    return "https://www.ncbi.nlm.nih.gov/bioproject/" + RegExp.$1;
  		}
  		    ...
  		    return "https://www.ncbi.nlm.nih.gov/gap/?term=" + RegExp.$1;
  		}
        else if(currDb == "dbSNP" && term.match(/^dbSNP\/(.+)/i)){
  		    return "https://www.ncbi.nlm.nih.gov/projects/SNP/snp_retrieve.cgi?subsnp_id=ss" + RegExp.$1;
  		}
        else if(currDb == "dbVar" && term.match(/^nstd(.+)/i)){
  		    return "https://www.ncbi.nlm.nih.gov/dbvar/studies/nstd" + RegExp.$1;
  		}
  		else if(currDb == "dbVar" && term.match(/^nsv(.+)/i)){
  		    return "https://www.ncbi.nlm.nih.gov/dbvar/variants/nsv" + RegExp.$1;
  		}
  		    ...
  		    return "https://www.ncbi.nlm.nih.gov/sra/" + RegExp.$1;
  		}
